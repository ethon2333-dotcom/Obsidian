...
- **摩擦反噬**：YubiKey 式动作级确认增加操作成本，低危高频动作若强制硬件审批会逼用户「为省事关掉确认」——分级必须到位（低危软件确认、高危硬件），否则安全机制被绕过。
- **密钥自身成靶面**：SharePoint CVE-2026-50522（数小时盗 machine key）、PyPI `mrmustard` 投毒证明「持有凭证的环节」本身就是高价值攻击目标；硬件审批把信任锚到密钥，密钥管理失误即全局击穿（呼应 [[Agent Data Injection 数据注入攻击]]）。
- **消费级应降配**：纯消费场景硬上硬件审批是过度工程，应允许软件 Confirmation UI 降配（见 [[Confirmation UI 安全机制]]）。
