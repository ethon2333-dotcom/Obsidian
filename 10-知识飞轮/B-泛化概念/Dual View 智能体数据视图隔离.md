...
- **与数据溯源棘轮互补**：DualView 隔离的是「数据可见形态」，[[数据溯源分级与单调棘轮]] 的「来源打标 + 单调棘轮」隔离的是「数据敏感度状态」——前者解决「Agent 看不看得到原文」，后者解决「看到后敏感度能否降级」。二者正交，可叠加成「符号化视图 + 不可降级敏感度」。
- **OS 一等公民建议的可行性**：笔记提出「Agent 视图文件系统」是否该做一等公民；据 [[数据溯源分级与单调棘轮]]，微软 Agent Governance Toolkit 已是开源治理层实现，但 Windows OS 内建仍空白——即工程可行、OS 采纳仍是产品优先级问题。
- **与 ADI 同源**：DualView 的符号化正是 ADI「伪造结构让 Agent 自推错误结论」的反面——把不可信内容恒为符号，Agent 无从伪造边界。

- [ ] 四平台（Apple / Android / Windows / HarmonyOS）是否有任何「Agent 视图文件系统」雏形？均待补。
- [ ] 符号化视图与结构化 Intent / Function 通道（[[Intent Schema Protocol 意图模式规范]]）如何共存？Agent 调工具时参数要不要也符号化？
- [ ] DualView 在移动端（小屏 / 受限算力）的可用性是否近基线？论文仅桌面验证，待补。
