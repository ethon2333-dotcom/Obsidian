...
- **基准口径提醒（延续笔记纪律）**：团队在 single-shot 场景自称跑赢 FunctionGemma-270M / Qwen-0.6B 等，但**非官方 BFCL 榜单行**；据 web 复核，BFCL 在 v3→v4 间测试集与口径发生过变动，跨版本分数**不可直接比较**，具体数值与测试集以官方榜为准（待核实）。
- **与端侧基准互补**：SAN 的「云端逃逸率」指标应放在 [[Local Agent Bench 端侧智能体基准]] 与 [[通用 AI Agent 评测基准 2026]] 的同一评估框架下对比，而非孤立看单模型速度。
- **架构外推**：「无 FFN」可推广到任何「模型能访问外部结构化知识」的任务，这与 [[Intent Schema Protocol 意图模式规范]] 的「Schema 越规范 Planner 越小」互为支撑。

- [ ] SAN 在「多轮 / 多工具串行依赖」场景的真实上限如何？团队自认「实验性单次调用」，需端侧基准实测（见 [[Local Agent Bench 端侧智能体基准]]）。
- [ ] 去掉 FFN 后，模型对「未见过的 tool schema」的泛化是否下降？待一手评测。
- [ ] INT4 14MB 能否进一步压到可穿戴/耳机端？量化噪声对槽位抽取精度的影响待测。
