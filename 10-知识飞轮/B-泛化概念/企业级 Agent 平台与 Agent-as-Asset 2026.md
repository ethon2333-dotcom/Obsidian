...
- **与意图支付授权衔接**：当企业 Agent 进入「交易 / 支付」动作，需把授权锚定到意图本身——中国银联 [[意图支付授权协议 APOP]] 的「意图即凭证」正对应 Agent-as-Asset 的「可治理资产」属性，可作为企业 Agent 资金动作的合规底座。
- **与 Skills 市场的同构**：Agent-as-Asset 的「可复用、可分发」与 [[Agent Skills 技能范式 2026]] 的 Skill 市场是同一战场——差别在企业平台偏「组织内资产」，Skill 偏「跨工具通用能力」。
- **隔离是基础**：阿里 MicroVM / VM 级强隔离与 OS 层 [[Agent Workspace 隔离执行]] 思路一致，且都要求「仿真评估前置」。

- [ ] Agent-as-Asset 的「资产确权 / 托管」在企业平台内具体由谁做？是否有链上 / 存证机制？
- [ ] 企业 Agent 跨平台迁移时，其治理元数据（身份 / 策略 / 审计）能否随 Agent 携带？需标准（呼应 [[智能体互联国家标准与 AIP]]）。
- [ ] 当 Agent 作为资产被多个业务调用时，其 provenance 链（见 [[数据溯源分级与单调棘轮]]）如何保证不被下游污染？
